...
- **Strip-the-anchors test.** Mentally delete every project-specific reference (paths, symbols, endpoints, business logic) from the memory's content. If what's left is still a useful standalone document, the project tie was decoration and the substance is tool/tooling knowledge — DROP, or UPDATE only if a rewrite around the actual project anchor produces something genuinely project-specific. **Internal or proprietary tools are not exempt:** memories describing how a private CLI, MCP server, GUI, or company-internal tool *works in general* belong in the tool's own docs or in `CLAUDE.local.md`, not here. Project endpoints sprinkled inside a tool how-to do not make it project knowledge.
